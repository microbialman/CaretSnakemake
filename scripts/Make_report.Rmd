...
params:
  classcol: Decline
  combinedlog: ~/devel/caret_pilot/test_mods/Report/combined.log
  metric: Kappa
  outdir: ~/devel/caret_pilot/test_mods
  structured: ~/devel/caret_pilot/test_mods/structured_data.rda
  configfile: ~/devel/caret_pilot/test.yml
---
...
A total of **`r nrow(comblog)`** models were run and **`r length(which(comblog$Status=="MODEL_COMPLETE"))`** completed training without errors.
...
getMetricsBest <- function(x){
  ...
  bestrow <- which(apply(paramstab, 1, function(x) all(x==bestparams)==TRUE))
  met <- as.list(x$results[bestrow,which(colnames(x$results)%in%metlist)])
  metsd <- as.list(x$results[bestrow,which(colnames(x$results)%in%paste0(metlist,"SD"))])
...
load(params$structured)

mcs <- function(x){
  predresp <- predict(x, newdata = test, type="raw")
  preddf <- data.frame(obs=pull(test,params$classcol),pred=predresp)
  predprob <- try(predict(x, newdata = test, type="prob"))
...
longscores <- bind_rows(predscores) %>% data.frame() %>% add_column(Metric=names(predscores[[1]])) %>% gather("Model","Test_Value",-Metric) %>% filter(!is.na(Test_Value)) %>% add_column(ModMet=paste0(.$Model,.$Metric))
