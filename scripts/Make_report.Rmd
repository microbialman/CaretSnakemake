...
longscores <- bind_rows(predscores) %>% data.frame() %>% add_column(Model=names(predscores)) %>% gather("Metric","Test_Value",-Model) %>% filter(!is.na(Test_Value)) %>% add_column(ModMet=paste0(.$Model,.$Metric))
